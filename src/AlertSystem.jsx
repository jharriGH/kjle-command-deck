...
const COOLDOWN_MS     = 600_000;     // 10 minutes — non-critical
const CRITICAL_REFIRE_MS = 300_000;  // 5 minutes — critical
const MAX_QUEUE       = 8;
...
function loadSession() {
  try {
    ...
    return raw ? JSON.parse(raw) : { alerts: [], cooldowns: {} };
  } catch { return { alerts: [], cooldowns: {} }; }
}
...
export function useAlerts() {
  ...
  const [alerts, setAlerts] = useState(
    Array.isArray(sessionInit.alerts) ? sessionInit.alerts : []
  );
  const cooldowns = useRef(sessionInit.cooldowns ?? {});
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (jobs.length === 0) {
          ...
          // Check if enrich_stage1 hasn't run in >24h
          ...
          // All jobs idle: last_ran > 13h ago
          const allOld = jobs.every(j => {
            if (!j.last_ran) return true;
            return Date.now() - new Date(j.last_ran).getTime() > 46_800_000;
          });
          if (allOld) {
            pending.push(mkAlert("scheduler_all_idle", "warn",
              "All scheduler jobs appear idle — check automation"));
          }
        }
      }
    } catch {}

    /* 4. GET /segments/summary — hot lead surge */
    try {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setAlerts(prev => prev.filter(a => {
        if (a.severity !== "info") return true;
        if (a.acknowledged) return false;
        return Date.now() - a.timestamp < 8_000;
      }));
    }, 1_000);
    return () => clearInterval(id);
  }, []);
...
/* ══════════════════════════════════════════════════════════════════════════
   AlertToastStack
   Fixed bottom-left, shows up to 6 unacknowledged alerts
══════════════════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════════════════
   AlertWarningStrip
   Collapsible strip between topbar and main grid
══════════════════════════════════════════════════════════════════════════ */
export function AlertWarningStrip({ alerts, acknowledgeAll, onHeightChange }) {
  ...
  useEffect(() => {
    ...
    const h = stripRef.current?.getBoundingClientRect().height
      ?? (expanded ? active.length * 22 + 28 : 28);
    onHeightChange(h);
